% TODO: explain the new arquitecture and cite both algorithms
% TODO: falar que olhamos na literatura e os artigos não mencionam como fizeram, e utilizamos os parâmetros defaults e escolhemos alguns que faziam mais sentido para SVM apenas
...
\begin{algorithm}
\label{algo:training}
\caption{Pseudo-Código de Treinamento dos Modelos Sem \texit{Tuning}}
\begin{algorithmic}
...
\caption{Pseudo-Código de Treinamento dos Modelos Com \texit{Tuning}}
\begin{algorithmic}
\REQUIRE $X$, $Y$, $k$ (número de divisões)
\ENSURE $[\hat{y_0}, ..., \hat{y_k}]$
\STATE $\hat{Y} \leftarrow [] $ 
