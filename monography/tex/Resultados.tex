...
\begin{enumerate}
	\item Número de divisões do conjunto de dados (\textit{Blocking}): Número de divisões utilizadas no blocking para criar mais subconjuntos de dados. Os valores testados foram: 1,2,4,8. Para cada subconjunto, a porcentagem utilizada para treinamento e teste foi a mesma: 80\% para treinamento e 20\% para testes.
	\item Passado Visível: Parâmetro que define qual a janela de tempo no passado que será visível no treinamento do modelo. O valor inicial considerado foi de 60 minutos. Os valores de 120, 240 e 480 minutos também foram testados.
	\item Tamanho do intervalo do fluxo: Parâmetro que define o intervalo de tempo no qual será acumulado a quantidade de veículos para o cálculo do fluxo. O valor inicial de teste foi 2,5 minutos. Foram realizados testes para as variações de 5 e 7,5 minutos também (ou 150, 300 e 450 segundos).
\end{enumerate}
...
Como pode ser observado no gráfico acima, os modelos de aprendizagem de máquina, no geral, tiveram um melhor desempenho com um número de divisões igual a 8. Dentre eles, o melhor desempenho foi o do \acrshort{RF} A. Este resultado já era esperado, pois com um número maior de treinos e testes, os modelos tendem a aprender melhor a distribuição dos dados, gerando melhores predições.
...
Quanto aos testes de variação do valor do passado visível, é perceptível que, no caso dos modelos tradicionais e de aprendizagem profunda, houve uma melhora na predição para todos eles à medida que o passada visível aumentava. Porém, vale notar que essa melhora não é tão significativa se for levado em conta que o eixo X do gráfico (Passado visível) tem seus valores aumentados de forma exponencial. Ou seja, para grandes variações no valor do passado visível, o erro das predições diminui em um ritmo muito menor. Dito isso, é válido salientar que aumentar o passado visível também aumenta o custo computacional do treinamento e, devido a este fato, não foram testados valores ainda maiores de passado visível.
...
Por último, pode-se notar que o tamanho do intervalo do fluxo adotado impacta de maneira significativa a qualidade das predições dos modelos. Em todos os casos, o erro quadrado médio das predições aumentou com intervalos de fluxo maiores sendo utilizados durante o treinamento. Isso se dá pelo fato de que ao se aumentar o intervalo de fluxo, diminui-se a quantidade de dados de entrada utilizados no treinamento dos modelos.
